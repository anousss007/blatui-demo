<x-layouts.app title="Chart Bar Interactive">
    <div class="flex min-h-svh items-center justify-center p-6">
        <div class="w-full max-w-2xl">
            <x-ui.card variant="sectioned" class="py-0"
                x-data="{
                    active: 'desktop',
                    chart: null,
                    ready: false,
                    ...
                    total(key) { return this.all.reduce((acc, p) => acc + p[key], 0); },
                    series() {
                        return [{
                            name: this.config[this.active].label,
                    ...
                    options() {
                        return window.Chart.buildChartOptions({
                            ...
                            colors: [this.config[this.active].color],
                            options: {
                                xaxis: { type: 'datetime', labels: { datetimeUTC: false, format: 'MMM d' }, axisBorder: { show: false }, axisTicks: { show: false }, minTickGap: 32 },
                                yaxis: { show: false },
                                legend: { show: false },
                                plotOptions: { bar: { columnWidth: '80%' } },
                                dataLabels: { enabled: false },
                                tooltip: { x: { format: 'MMM dd, yyyy' } },
                                grid: { strokeDashArray: 0, yaxis: { lines: { show: true } }, xaxis: { lines: { show: false } } },
                            },
                        });
                    },
                    init() {
                    ...
                    setActive(key) { this.active = key; if (this.chart && this.ready) this.chart.updateOptions(this.options(), false, false); },
                }">
                <x-ui.card-header class="flex flex-col items-stretch border-b !p-0 sm:flex-row">
                    <div class="flex flex-1 flex-col justify-center gap-1 px-6 pt-4 pb-3 sm:!py-0">
                        <x-ui.card-title>Bar Chart - Interactive</x-ui.card-title>
                ...
                <x-ui.card-content class="px-2 sm:p-6">
                    <div
                        data-slot="chart"
                        class="flex aspect-auto h-[250px] w-full justify-center text-xs [&_.apexcharts-tooltip]:!rounded-lg [&_.apexcharts-tooltip]:!border [&_.apexcharts-tooltip]:!border-border [&_.apexcharts-tooltip]:!bg-popover [&_.apexcharts-tooltip]:!text-popover-foreground [&_.apexcharts-tooltip]:!shadow-xl"
                    >
